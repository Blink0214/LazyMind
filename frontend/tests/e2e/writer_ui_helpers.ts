...
const WORKFLOW_PANEL = ".workflow-panel[data-session-id]";
...
export async function openWriterConversation(page: Page, conversationId: string) {
  ...
  const panel = page.locator(WORKFLOW_PANEL).first();
  await expect(panel).toBeVisible({ timeout: 180_000 });
  return panel;
}

export type TextSelectionEvidence = {
  selectedText: string;
  blockText: string;
};

export async function selectTextInFirstBlock(
  root: Locator,
  selector: string,
): Promise<TextSelectionEvidence> {
  const block = root.locator(selector).filter({ hasText: /\S/ }).first();
  ...
  return block.evaluate((element) => {
    ...
    return {
      selectedText: textNode.textContent.slice(start, end),
      blockText: (element as HTMLElement).innerText,
    };
  });
}
