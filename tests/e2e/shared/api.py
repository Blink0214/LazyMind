...
Default chunk size is 5 MB to match the Writer upload path.
...
def normalize_session_response(payload: dict | None) -> dict:
    """Extract the current Core ``data.session`` response contract."""
    data = payload.get("data") if isinstance(payload, dict) else None
    session = data.get("session") if isinstance(data, dict) else None
    return session if isinstance(session, dict) else {}


def fetch_workflow_session(base_url: str, token: str, session_id: str) -> dict:
    """Return the detailed workflow session, including step attempts."""
    from urllib.parse import quote
    encoded = quote(str(session_id), safe="")
    req = urllib.request.Request(
        f"{base_url.rstrip('/')}/api/core/workflow-sessions/{encoded}",
        headers={"Authorization": f"Bearer {token}"},
        method="GET",
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
        payload = json.loads(resp.read().decode("utf-8"))
    session = normalize_session_response(payload)
    if not session:
        raise RuntimeError(f"workflow session not found: {session_id}")
    return session
...
def _latest_session(base_url, token, conversation_id) -> dict | None:
    ...
    return normalize_session_response(response) or None
